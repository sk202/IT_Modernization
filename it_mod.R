...
setwd("C:/Users/StephenFKeller/Desktop/CART and FPDS Files/it_modernization-master/data")
library(dplyr)
...
business<-
  read.csv("https://itdashboard.gov/api/v1/ITDB2/dataFeeds/businessCase?csv=true",
           stringsAsFactors = FALSE,strip.white=TRUE)
...
#                    stringsAsFactors = FALSE)
#https://itdashboard.gov/api/v1/ITDB2/dataFeeds/contractsReport?csv=true
contracts<-
  read.csv("https://itdashboard.gov/api/v1/ITDB2/dataFeeds/contractsReport?csv=true",
           stringsAsFactors = FALSE,strip.white=TRUE)
...
portfolio<-
  read.csv("https://itdashboard.gov/api/v1/ITDB2/dataFeeds/portfolio?csv=true",
           stringsAsFactors = FALSE,strip.white=TRUE)
...
write.csv(portfolio_contracts,file="ITC_portfolio.csv")

#open and explore file in tableau
portfolio_contracts<-read.csv("ITC_portfolio.csv",
                              stringsAsFactors = FALSE, 
                              header = TRUE)
...
#write out summary
write.csv(portfolio_summary,file="portfolio_summary.csv")
...
#read final files
write.csv(counts_mod,"counts_mod.csv")
write.csv(counts_major,"counts_major.csv")
write.csv(agency_counts_mod,"agency_counts_mod.csv")
write.csv(agency_counts_major,"agency_counts_major.csv")